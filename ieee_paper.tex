...
one-month forecasts with ${\sim}10\times$ less memory than a reservoir.
\end{abstract}
...
($a_{\mathrm{base}} \propto N/49$; 5 seeds per size; Fig.~\ref{fig:scale}). Without
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figs/fig5_scaling.png}
\caption{Scaling with constant power density (5 seeds per size). (a) Identification
advantage persists. (b) Self-organization improves. (c) Memory advantage constant.}
\label{fig:scale}
\end{figure}
...
SINDy, AR(24), an echo state network (400 neurons), and persistence.
...
1.25 MB). Honest boundary: AR(24)/ESN lead at long horizons ($0.177/0.166$ vs.\
$0.296$ at 12 months), because derivative laws carry almost no level information
and the solar cycle drifts across decades. A derivative-law identifier is a robust
law extractor, not a long-horizon forecaster -- and that is what Table~\ref{tab:real-id}
supports.
...
\toprule
horizon & weak & FD str. & b.weak & b.FD & AR(24) & ESN & pers. \\
\midrule
...
potential, 12/15 seeds). (ii) Corridor specificity and routing weaken at scale.
(iii) Validation is 49--225-node simulation; no physical implementation was
fabricated -- the hardware mapping (Sec.~III-H) is a design rule, not a measurement.
(iv) The 589$\times$ memory ratio compares against a frame-buffer batch estimator;
streaming batch solvers would close part of the gap, but not the \emph{error} gap,
...
\Delta t I)$, which is unconditionally stable.
